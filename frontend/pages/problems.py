...
def render_problem_detail(api: ApiClient, problem_id: str) -> None:
    ...
    if meta:
        st.caption(meta)
    if problem.get("tags"):
        st.caption("标签：" + "、".join(problem["tags"]))
    ...
    st.write(
        f"时间限制：{problem['time_limit']} 秒；内存限制：{problem['memory_limit']} MB"
    )
    if problem.get("hint"):
...
def render_problem_management(api: ApiClient, is_admin: bool) -> None:
    ...
    mode = st.radio("操作", options, horizontal=True)
    initial = None
    problem_id = ""
    if mode in {"编辑", "删除"}:
