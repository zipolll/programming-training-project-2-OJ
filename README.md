...
详情默认以题目为主，AI 修改按需展开右侧面板，窄屏切换为全宽面板。手动编辑分三个分组。
任务概览与题目工作区使用上下两个白色容器；原始需求独立展示摘要并可展开，不作为标题渲染。
导入通过确认弹窗完成。终态不显示进度条，执行历史、要求、日志与费用收于“任务信息”。
可沿用历史要求创建独立记录；失败后仍可打开之前的可用版本。手动内容必须重新验证才能导入。
