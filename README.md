...
- `POST /api/agent/tasks/{task_id}/validate`：原地验证所选版本，返回相同任务 ID 和版本号。
  ...
  验证执行单独记录入队时间，旧的手动稿也有完整四分钟验证预算，不重置版本创建时间。
...
目录执行并自动清理。每次执行从创建入队起共享 **240 秒总时限**，覆盖排队、生成、
JSON 修复、质量修正和验证。到期取消正在进行的请求或评测，返回 `task_timeout`，
保留草稿、验证结果及已知用量；生成失败重试创建独立执行。
...
5 个精简测试点及一个典型错误解法，显式指定的测试点数量仍优先。
