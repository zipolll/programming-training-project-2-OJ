...
- `POST /api/agent/tasks/{task_id}/versions`：提交完整 `generated`（GeneratedProblem）
  及可选 `validate`。仅内容发生变化时手动保存另建版本；无改动保存返回原版本。
  `stage=awaiting_validation` 表示待验证，此时
  无验证报告且不可导入；`validate=true` 同时排队验证，完全不调用模型或改写内容。
- `POST /api/agent/tasks/{task_id}/validate`：原地验证所选版本，返回相同任务 ID 和版本号。
  验证通过后同一版本变为已完成；失败保留内容，重新验证也不增加版本。
  重复点击不会并行验证，已通过验证的未修改版本直接返回已完成。
  验证执行单独记录入队时间，旧的手动稿也有完整八分钟验证预算，不重置版本创建时间。
  内容版本摘要位于详情的 `versions`，生成与修订执行摘要位于 `attempts`；
  没有生成内容的执行及 AI 未改变内容的执行不增加内容版本数。
...
入口保留在“命题中心 → AI 智能命题”，导航为“新建出题 / 出题记录”，模型配置位于右侧。
新建页直接描述需求或设置固定条件，点击记录名称进入详情，修改、重试等操作集中在详情。
详情使用上下白色容器：任务概览、题目工作区；进入修改后追加“与 AI 讨论修改”容器，
AI 消息居左、用户消息居右。展示页只呈现题目与任务信息；修改页隐藏任务信息，
验证、保存、另存为新版本位于对话输入框下方，验证结果也在此展示。手动编辑分三个分组。
原始需求独立展示摘要并可展开，不作为标题渲染。导入通过确认弹窗完成。终态不显示进度条，
失败后可在工作区调整要求重试，仍可打开之前的可用版本。手动内容必须重新验证才能导入。
记录摘要保留原 `status`，新增 `display_status`，已导入的最新成功版本为 `imported`，
其他值与 `status` 一致。`GET /api/agent/records` 支持同名筛选参数，在分页前过滤；原
`status` 筛选语义不变。旧版本导入后产生新版本，展示新版本状态，历史导入信息保留在详情。
...
目录执行并自动清理。每次执行从创建入队起共享 **480 秒总时限**，覆盖排队、生成、
大数据构造、JSON 修复、质量修正和验证。到期取消正在进行的请求或评测，返回
`task_timeout`，保留草稿、验证结果及已知用量；生成失败重试创建独立执行。
纯验证重试更新原版本，先前验证报告及错误保留在验证事件中。
中断请求未返回的用量无法准确结算，最终费用以服务商账单为准。
GLM-5.3 / GLM-5.3-FLASH 使用 `thinking.type=enabled` 和 `reasoning_effort=low`；
其他模型不附加这两个专用参数。默认单请求超时为 180 秒，旧配置中超过 240 秒的
请求超时在执行时截断为 240 秒，任何配置都不能延长任务总时限。
实际单次输出上限为配置值和 16000 Token 中的较小值；未指定测试点数量时要求
5 个精简手写测试点及一个典型错误解法，显式指定的测试点数量仍优先。
检索先筛选摘要，只加载命中的题目详情；样例与测试点合并验证，共用一次参考解法
编译并复用相同输入输出。参考解法和反例验证最多并行运行两项，取消时一并回收。
验证失败会把结构化报告反馈给模型，在总时限内默认最多验证三轮；仍失败
则保留报告并禁止导入。

### 强数据出题（防暴力超时）

需要用数据规模区分算法复杂度的题目（区间查询、集合/字典判定、优先队列调度、
扫描线等）不再让模型逐字输出大数组——那受输出 Token 上限约束必然偏小。改为
**数据生成器方案**：

- 模型返回 `testcase_generator`：一段自包含的 Python 生成器
  （`build_case(spec) -> str`，按 `seed` 确定性随机）和 3–8 个带不同种子与
  参数的测试点说明；手写用例仍保持小而精（边界、最小规模、对抗数据）。
- 后端在沙箱内逐个执行生成器得到大规模输入（单点上限 8 MB、20 秒），
  再用参考程序推导期望输出，随后清空生成器字段、把物化测试点并入题目。
  生成器崩溃、参考程序超时或期望输出超过判题输出捕获上限时，结构化错误
  会反馈给模型自动修复。
- 输出体积预算：每点期望输出默认须控制在约 60 KB 内（判题 stdout 捕获上限
  64 KiB，超限判 WA）；确需逐项输出的题目可在 `problem.output_limit_bytes`
  （4096–8388608）按题提高捕获上限，判题器按题覆盖全局默认值。
- 需求勾选“强数据（防暴力超时）”或题目命中常见复杂度区分题型
  （`backend/app/modules/agent/archetypes.py` 按 prompt/知识点/数据规模关键词
  静默识别）时，验证阶段强制执行强数据门禁：最大测试点输入不足 50 KB、
  或没有任何错误解法在大数据上超时（TLE），都会作为阻断错误要求模型返工，
  同时提示词附带该题型的参考数据规模、应拒绝的暴力解法与输出预算。
- 物化后的大测试点在预览与编辑器中自动折叠显示（长度、参考耗时、截断预览），
  未修改的超长单元格保存时自动还原，避免界面渲染兆级文本。
- 修订回传给模型的上一稿会折叠超大测试数据（标记 `__TRUNCATED_LARGE_CASE__`），
  防止大数据撑爆模型上下文。
